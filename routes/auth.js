...
router.post('/verificar-pregunta', async (req, res) => {
  const { email, pregunta_seguridad, respuesta_seguridad, nueva_password } = req.body;
  const hashedRespuesta = hashPassword(respuesta_seguridad.trim().toLowerCase());
  const hashedPassword = hashPassword(nueva_password);
  const pool = await poolPromise;
  // Busca usuario con email, pregunta y respuesta
  const result = await pool.request()
    .input('email', email)
    .input('pregunta_seguridad', pregunta_seguridad)
    .input('respuesta_seguridad', hashedRespuesta)
